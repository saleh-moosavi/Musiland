...
import { addPlaylistSchema } from "@/constants/zodSchema";
import CustomInput from "@/components/auth/CustomInput";
import { useRouter, useSearchParams } from "next/navigation";

type Mode = "add" | "edit";

export default function PlaylistForm({ mode }: { mode: Mode }) {
  ...
  useEffect(() => {
    if (mode === "edit" && playlistName) {
      reset({ name: playlistName });
    }
  }, [mode, playlistName, reset]);
  ...
  const onSubmit = async (data: { name: string }) => {
    ...
    const url =
      mode === "add"
        ...
        : `${process.env.NEXT_PUBLIC_API_URL}/playlists/edit/${playlistId}`;
    ...
    try {
      ...
      if (res.ok) {
        reset();
        alert(`playlist ${mode === "add" ? "added" : "updated"} successfully!`);
        router.push("/admin/dashboard/playlist");
      } else {
    ...
    <article className="max-w-md min-w-md mx-auto overflow-hidden p-5 rounded-3xl bg-slate-100 dark:bg-slate-800 mt-10 shadow dark:shadow-white/30 text-black dark:text-white">
      <form onSubmit={handleSubmit(onSubmit)} className="grid gap-y-10">
        <h3 className="text-center">
          {mode === "add" ? "Add" : "Edit"} Playlist
        </h3>

        <CustomInput
          register={register("name")}
          icon={<UserIcon className={iconClasses} />}
          name="Enter Name of Playlist"
          error={
